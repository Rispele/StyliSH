...
public static class Class1
{
    public static void Main()
    {
        GenDomainResult<string> alias = GenDomainResultMarker.Pure(1)
            .Map(t => t * 10, out var multiplied)
            .Map(t => "123", out var value123)
            .Bind(t => GenDomainResultMarker.Pure($"Треш: {value123.Value} {multiplied.Value}"));
        EitherT<IdMarker, string, string> transformer = alias.Inner.Wrap();
        var monads = transformer.Run<IdMonad<Either<string, string>>>();
        var value = monads.ActualValue.Match(
            s => "Error" + s,
            i => "Result" + i);
